...
class Game
  ...
  class GameText
    def initialize(args, text, y_pos=HALF_SCREEN_HEIGHT, x_pos=HALF_SCREEN_WIDTH, line_skip=50)
      @args = args
      @x_pos = x_pos
      @y_pos = y_pos
      @anchors = 1.5
...
#HELPER FUNCTIONS
  ...
  def enemy_respawn(enemy)
    enemy.y_pos = SCREEN_HEIGHT
    enemy.x_pos = rand(SCREEN_HEIGHT)
    enemy.speed = rand(5) + 6
