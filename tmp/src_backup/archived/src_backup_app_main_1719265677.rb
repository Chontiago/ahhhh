...
ENEMY_Y_POSITION = SCREEN_HEIGHT - CUBE_SIZE
...
class Game
  def initialize(args)
    ...
    @player = Cube.new(PLAYER_X_POSITION, 0, PLAYER_WIDTH).draw_cube
    @enemy = Cube.new(@randomize_x_pos, ENEMY_Y_POS, CUBE_SIZE ).draw_cube
    @move ||= nil
    @score ||= 0
    @fall_speed  ||= 5
  end
