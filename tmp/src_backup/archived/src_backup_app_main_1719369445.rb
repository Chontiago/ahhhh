...
class Game
  def initialize(args)
    ...
    @enemy = Box.new(rand(SCREEN_WIDTH), ENEMY_SPAWN_POINT, BOX_SIZE ).draw_box
    @enemies = Box.new(rand(SCREEN_WIDTH), ENEMY_SPAWN_POINT, BOX_SIZE ).draw_enemies
    @score ||= 0
  ...
  class Box
    ...
    def draw_box
      return {x: @x, y: @y, w: @width , h: @height, fallspeed: @fallspeed}
    end
    ...
    def draw_enemies
      ...
      10.times do 
       enemies << Box.new(rand(HALF_SCREEN_WIDTH),@y, @width).draw_box
      end
  ...
  def player_bounds
    @player.x = [@player.x, LEFT_BORDER].max
    @player.x = [@player.x, RIGHT_BORDER].min
  end
  ...
  def score
    if @enemy.y <= BOTTOM_BORDER
      ...
      @enemy.fallspeed = rand(6)
      @score += 1
    end
  end

  def game_over
    if collision(@enemies, @player)
      @game_over = true
    end
  end
