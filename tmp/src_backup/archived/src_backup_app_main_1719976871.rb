...
class Game
  def initialize(args)
    ...
    @enemy ||= Enemies.new(HALF_SCREEN_WIDTH, SCREEN_HEIGHT, CUBE_SIZE, CUBE_SIZE)
    @score ||= 0
    @score_text = GameText.new(@args, "SCORE: #{@score}", SCREEN_HEIGHT, RIGHT_BORDER)
  end
  ...
  def render_gameplay
    ...
    @args.outputs.labels << {x:HALF_SCREEN_WIDTH, y: HALF_SCREEN_HEIGHT , anchor_x: 0.5, anchor_y: 0.5, text: @enemy.y_pos}
    @args.outputs.labels << {x:HALF_SCREEN_WIDTH, y: HALF_SCREEN_HEIGHT - 100, anchor_x: 0.5, anchor_y: 0.5, text: @player.traits.to_s}
  end
  ...
  def score
    if @enemy.traits.y <= BOTTOM_BORDER
      @score += 1
    end
  end

  def game_over
    if collision(@player.traits, @enemy.traits)
      @game_over = true
    end
  end
