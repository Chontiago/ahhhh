...
class Game
  def initialize(args)
    ...
    @enemy = Enemies.new(args, rand(SCREEN_WIDTH), HALF_SCREEN_HEIGHT, CUBE_SIZE, CUBE_SIZE)
    @score ||= 0 
  end
  ...
  class Enemies
    ...
    def initialize(args, x_pos, y_pos, w, h)
      @args = args
      @x_pos = x_pos
      @y_pos = y_pos
      @width = w
  ...
  def generate_enemies(quantity)
    enemies = []
    quantity.times do
      #enemies << Enemies.new(rand(HALF_SCREEN_WIDTH), HALF_SCREEN_HEIGHT, CUBE_SIZE, CUBE_SIZE).traits
      enemies << @enemy.traits
    end
    return enemies 
  end
