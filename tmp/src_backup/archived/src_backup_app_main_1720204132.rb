...
class Game
  def initialize(args)
    ...
    @enemies ||= Enemies.multiple_enemies(SCREEN_WIDTH, HALF_SCREEN_HEIGHT, CUBE_SIZE, CUBE_SIZE)
    @score ||= 0 
  end
  ...
  class Enemies
    ...
    def self.multiple_enemies(x_pos, y_pos, width, height)
      enemies = []

     
      10.times do
        enemies << Enemies.new(rand(x_pos), y_pos, width, height).traits
      end
  ...
  def score
    if @enemies.y_pos <= BOTTOM_BORDER
      @score += 1
      @enemies.y_pos = SCREEN_HEIGHT
    end
  end
