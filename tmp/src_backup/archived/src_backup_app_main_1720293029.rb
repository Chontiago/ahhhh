...
class Game
  def initialize(args)
    ...
    @enemy ||= Enemies.new(rand(SCREEN_WIDTH), HALF_SCREEN_HEIGHT, CUBE_SIZE, CUBE_SIZE)
    @score ||= 0 
  ...
  def enemy_movement
    single_enemy.y_pos -= single_enemy.speed
  end
  
  def score
    if single_enemy.y_pos <= BOTTOM_BORDER
      @score += 1
    end
  end

  def game_over
    if collision(single_enemy.traits, @player.traits)
     @game_over = true
    end
  end
  ...
  def generate_enemies(number_of_enemies)
    ...
    number_of_enemies.times do
      enemies << Enemies.new(rand(SCREEN_WIDTH), HALF_SCREEN_HEIGHT, CUBE_SIZE, CUBE_SIZE)
    end
    return enemies
  end
